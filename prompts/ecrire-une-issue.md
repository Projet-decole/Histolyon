...
- **« À lire »** : des IDs de `conception/` (la liste est dans `conception/INDEX.md`, généré — ne pas l'éditer) et des chemins de fichiers précis. Jamais « lis la conception ».
...
- **Labels** : la zone (`mobile`, `admin`, `supabase`, `content`, `3d`), le domaine de conception (`D1`…`D11`, ou `socle`) pour les indicateurs du rapport (AD-17), plus `prête` quand l'issue est complète et que ses dépendances sont fusionnées.
