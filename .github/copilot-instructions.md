...
Détail par domaine : `apps/mobile/AGENTS.md`, `supabase/AGENTS.md`.
...
- `docs/` — L1, porte d'entrée après `AGENTS.md` : `docs/ORGANISATION.md` (comment et pourquoi le dépôt est organisé ainsi — à lire en premier), `docs/BOOTSTRAP.md` (comment le socle a été construit), `docs/ARCHITECTURE-SPINE.md` (les 20 règles `AD-n`), `docs/DECISIONS.md` (pourquoi chaque règle existe, journal chronologique), `conventions/` et `guides/` (un sujet par fichier), `docs/specs/` et `docs/stories/` (historique du bootstrap, voir `docs/stories/README.md`).
