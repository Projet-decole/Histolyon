...
- Versions figées à vérifier : `flutter_lints` 6. `riverpod_lint` différé à l'Epic 6 (conflit `cli_util` avec `melos` dans le workspace Pub natif, vérifié empiriquement en Story 1.5 — voir `BOOTSTRAP.md` §1 et `spec-1-5-lint-d-imports-et-lints-de-base.md`). Toute autre techno/version passe par amendement d'architecture.
