...
- `tools/` — scripts d'outillage. Existent aujourd'hui : `check_import_direction.dart`. Le reste (`ctx`, `gen-types`, `sync-agents`, `seed`, `media`, `tiles`, `check-tokens`, `report`) arrive épic par épic — ne pas les traiter comme déjà là avant leur story.
